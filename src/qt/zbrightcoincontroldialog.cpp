...
#include "ui_zbrightcoincontroldialog.h"
...
std::list<std::string> ZBrightCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBrightCoinControlDialog::listMints;

ZBrightCoinControlDialog::ZBrightCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBrightCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBrightCoinControlDialog::~ZBrightCoinControlDialog()
{
    delete ui;
}

void ZBrightCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBrightCoinControlDialog::updateList()
{
...
void ZBrightCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBrightCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBrightCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBrightCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBrightCoinControlDialog::GetSelectedMints()
{
...
void ZBrightCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
